    ...
    <AppLayout>
      ...
      <div style={styles.header}>
        <div>
          ...
          <p style={styles.subtitle}>
            Welcome, {user?.name || 'Responder'} — stay alert
          </p>
        </div>
      ...
      <div style={styles.mapSection}>
        <div style={styles.mapHeader}>
          <h2 style={styles.sectionTitle}>Live SOS map</h2>
          <span style={styles.mapCount}>
            {sosList.filter(s => s.status === 'PENDING').length} pending SOS
          </span>
        </div>
        <DisasterMap sosAlerts={sosList} height="280px" />
      </div>
            ...
            {!loading && sosList.map((sos) => {
              ...
              return (
                <div key={sos.id} style={styles.sosItem}>
                  ...
                  {sos.status === 'RESOLVED' && (
                    <div style={styles.resolvedTag}>Resolved</div>
                  )}
                </div>
              );
            })}
...
const styles = {
  ...
  dutyBadge: { display: 'flex', alignItems: 'center', gap: '6px',
    fontSize: '13px', color: '#639922', background: '#EAF3DE',
    padding: '6px 12px', borderRadius: '20px', border: '0.5px solid #C0DD97' },
  ...
  mapSection: { background: '#ffffff', borderRadius: '12px',
    border: '0.5px solid #e0dfd7', padding: '20px', marginBottom: '20px' },
  mapHeader: { display: 'flex', justifyContent: 'space-between',
    alignItems: 'center', marginBottom: '14px' },
  mapCount: { fontSize: '12px', color: '#A32D2D',
    background: '#FCEBEB', padding: '3px 10px', borderRadius: '20px' },
  ...
  acceptBtn: { flex: 1, padding: '8px', background: '#E24B4A',
    color: '#fff', border: 'none', borderRadius: '6px',
    fontSize: '12px', fontWeight: '500', cursor: 'pointer' },
  resolveBtn: { flex: 1, padding: '8px', background: '#639922',
    color: '#fff', border: 'none', borderRadius: '6px',
    fontSize: '12px', fontWeight: '500', cursor: 'pointer' },
  ...
  onlineDot: { width: '8px', height: '8px',
